...
csv_file_path = '../csv/CanUK.csv'
json_file_path = '../json/CanUK.json'
...
def download_and_scrape_example(seed, cookie=None):
    ...
    for tr in soup.findAll('table')[0].tbody.findAll('td'):
        print(tr.text.strip())
        CanUK_list.append(tr.text.strip())
    ...
    for item in CanUK_list:
        if iterator % 3 == 0:
           i = iterator // 3
           product = 'Product' + str(i)
           CanUK_dictionary[product] = item
           iterator += 1
        elif iterator % 3 == 1:
            i = iterator // 3
            price = 'Price' + str(i)
            CanUK_dictionary[price] = item
            iterator += 1
        elif iterator % 3 == 2:
            i = iterator // 3
            availability = 'Quantity' + str(i)
            CanUK_dictionary[availability] = item
            iterator += 1
    print(CanUK_dictionary)
...
def scrape_example(url_path):
    ...
    for filename in os.listdir(url_path):
        ...
        if filename.endswith(".html"):
            ...
            for item in CanUK_list:
                ...
                CanUK_dictionary[key] = item
                iterator += 1

    print(CanUK_dictionary)
    # Extract keys and values
    keys = [key for key in CanUK_dictionary.keys()]
    values = [CanUK_dictionary[key] for key in keys]
    ...
    with open(json_file_path, 'w') as jsonfile:
        json.dump(CanUK_dictionary, jsonfile, indent=4)
