...
csv_file_path = '../csv/EuCanna.csv'
json_file_path = '../json/EuCanna.json'
...
def download_and_scrape_example(seed, cookie=None):
    ...
    for tr in soup.findAll('table')[0].tbody.findAll('td'):
        print(tr.text.strip())
        EuCanna_list.append(tr.text.strip())
    ...
    for item in EuCanna_list:
        if iterator % 3 == 0:
           i = iterator // 3
           product = 'Product' + str(i)
           EuCanna_dictionary[product] = item
           iterator += 1
        elif iterator % 3 == 1:
            i = iterator // 3
            price = 'Price' + str(i)
            EuCanna_dictionary[price] = item
            iterator += 1
        elif iterator % 3 == 2:
            i = iterator // 3
            availability = 'Quantity' + str(i)
            EuCanna_dictionary[availability] = item
            iterator += 1
    print(EuCanna_dictionary)
...
def scrape_example(url_path):
    ...
    for filename in os.listdir(url_path):
        ...
        if filename.endswith(".html"):
            ...
            for item in EuCanna_list:
                ...
                EuCanna_dictionary[key] = item
                iterator += 1

    print(EuCanna_dictionary)
    # Extract keys and values
    keys = [key for key in EuCanna_dictionary.keys()]
    values = [EuCanna_dictionary[key] for key in keys]
    ...
    with open(json_file_path, 'w') as jsonfile:
        json.dump(EuCanna_dictionary, jsonfile, indent=4)
