...
csv_file_path = '../csv/pds.csv'
json_file_path = '../json/pds.json'
...
def download_and_scrape_example(seed, cookie=None):
    ...
    for item in pds_list:
        if iterator % 3 == 0:
           ...
           pds_dictionary[product] = item
           iterator += 1
        elif iterator % 3 == 1:
            i = iterator // 3
            price = 'Price' + str(i)
            pds_dictionary[price] = item
            iterator += 1
        elif iterator % 3 == 2:
            i = iterator // 3
            availability = 'Availability' + str(i)
            pds_dictionary[availability] = item
            iterator += 1
    print(pds_dictionary)
...
def scrape_example(url_path):
    ...
    for filename in os.listdir(url_path):
        ...
        if filename.endswith(".html"):
            ...
            for item in pds_list:
                ...
                pds_dictionary[key] = item
                iterator += 1

    print(pds_dictionary)
    # Extract keys and values
    keys = [key for key in pds_dictionary.keys()]
    values = [pds_dictionary[key] for key in keys]
    ...
    with open(json_file_path, 'w') as jsonfile:
        json.dump(pds_dictionary, jsonfile, indent=4)
