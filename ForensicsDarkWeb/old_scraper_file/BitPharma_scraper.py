...
csv_file_path = '../csv/BitPharma.csv'
json_file_path = '../json/BitPharma.json'
...
def download_and_scrape_example(seed, cookie=None):
    ...
    for tr in soup.findAll('table')[0].tbody.findAll('td'):
        print(tr.text.strip())
        bitpharma_list.append(tr.text.strip())

    # Creating unique products, prices and availability
    bitpharma_dictionary = {}
    iterator = 0

    #Since we have 3 categories (Product, Price, Availability), we can divide them by 3

    for item in bitpharma_list:
        if iterator % 3 == 0:
           i = iterator // 3
           product = 'Product' + str(i)
           bitpharma_dictionary[product] = item
           iterator += 1
        elif iterator % 3 == 1:
            i = iterator // 3
            price = 'Price' + str(i)
            bitpharma_dictionary[price] = item
            iterator += 1
        elif iterator % 3 == 2:
            i = iterator // 3
            availability = 'Quantity' + str(i)
            bitpharma_dictionary[availability] = item
            iterator += 1
    print(bitpharma_dictionary)
...
def scrape_example(url_path):
    ...
    for filename in os.listdir(url_path):
        ...
        if filename.endswith(".html"):
            ...
            for item in bitpharma_list:
                ...
                bitpharma_dictionary[key] = item
                iterator += 1

    print(bitpharma_dictionary)
    # Extract keys and values
    keys = [key for key in bitpharma_dictionary.keys()]
    values = [bitpharma_dictionary[key] for key in keys]
    ...
    with open(json_file_path, 'w') as jsonfile:
        json.dump(bitpharma_dictionary, jsonfile, indent=4)
