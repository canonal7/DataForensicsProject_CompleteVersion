...
csv_file_path = '../csv/Smokeables.csv'
json_file_path = '../json/Smokeables.json'
...
def download_and_scrape_example(seed, cookie=None):
    ...
    for tr in soup.findAll('table')[0].tbody.findAll('td'):
        print(tr.text.strip())
        Smokeables_list.append(tr.text.strip())

    # Creating unique products, prices and availability
    Smokeables_dictionary = {}
    iterator = 0

    #Since we have 3 categories (Product, Price, Availability), we can divide them by 3

    for item in Smokeables_list:
        if iterator % 3 == 0:
           i = iterator // 3
           product = 'Product' + str(i)
           Smokeables_dictionary[product] = item
           iterator += 1
        elif iterator % 3 == 1:
            i = iterator // 3
            price = 'Price' + str(i)
            Smokeables_dictionary[price] = item
            iterator += 1
        elif iterator % 3 == 2:
            i = iterator // 3
            availability = 'Quantity' + str(i)
            Smokeables_dictionary[availability] = item
            iterator += 1
    print(Smokeables_dictionary)
...
def scrape_example(url_path):
    ...
    for filename in os.listdir(url_path):
        ...
        if filename.endswith(".html"):
            ...
            for item in Smokeables_list:
                ...
                Smokeables_dictionary[key] = item
                iterator += 1

    print(Smokeables_dictionary)
    # Extract keys and values
    keys = [key for key in Smokeables_dictionary.keys()]
    values = [Smokeables_dictionary[key] for key in keys]
    ...
    with open(json_file_path, 'w') as jsonfile:
        json.dump(Smokeables_dictionary, jsonfile, indent=4)
